...
app/
  ...
  api/        Route nghiệp vụ riêng: import-players
...
**Cầu thủ** có 2 nguồn dữ liệu, đánh dấu bằng field `Nguồn dữ liệu`:

1. **Nhập tay** — tạo trực tiếp trong admin.
2. **Nhập từ Excel** — nút *Import Excel*; tải file mẫu tại `/api/import-players/template`. Trùng tên sẽ bị bỏ qua, lỗi báo theo từng dòng.

File mẫu gồm 7 cột: *Tên cầu thủ*, *Số áo*, *Vị trí* (bắt buộc) và *Quốc tịch*, *Ngày sinh*, *Chiều cao (cm)*, *Cân nặng (kg)* (bỏ trống được). Ngày sinh nhận `dd/MM/yyyy`, `dd-MM-yyyy`, `yyyy-MM-dd` hoặc ô đã định dạng ngày của Excel; giá trị không đọc được thì bỏ qua ô đó chứ không loại cả dòng.
